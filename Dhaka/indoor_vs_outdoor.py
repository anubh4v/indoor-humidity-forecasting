...
indoor_input_file = os.path.join(script_dir, 'Dhaka Indoor Data.csv')
indoor_df = pd.read_csv(indoor_input_file)

# Read the outdoor CSV file
outdoor_input_file = os.path.join(script_dir, 'output.csv')
...
# ============================================================================
# Create masks for different date formats in outdoor data
outdoor_mask1 = outdoor_df['DD/MM/YYYY'].str.contains(r'^\d{2}/\d{2}/\d{4}$', regex=True, na=False)  # DD/MM/YYYY format
outdoor_mask2 = outdoor_df['DD/MM/YYYY'].str.contains(r'^\d{2}-\d{2}-\d{4}$', regex=True, na=False)    # MM-DD-YYYY format
outdoor_mask3 = outdoor_df['DD/MM/YYYY'].str.contains(r'^\d{1,2}-\d{1,2}-\d{2}$', regex=True, na=False)  # D-M-YY format
...
outdoor_df.loc[outdoor_mask3, 'Datetime'] = pd.to_datetime(
    outdoor_df.loc[outdoor_mask3, 'DD/MM/YYYY'] + ' ' + outdoor_df.loc[outdoor_mask3, 'Time'], 
    format='%d-%m-%y %H:%M:%S',
    errors='coerce'
)

# Check for unmatched outdoor rows
outdoor_unmatched = ~(outdoor_mask1 | outdoor_mask2 | outdoor_mask3)
